...
describe('littleLisp', function() {
  ...
  describe('alpha reduction', function() {
    it('should return list of identifiers', function() {
      expect(unannotate(t.alphaConvert(t.parse('(a b 3)')))).toEqual(["a", "b", "3"]);
    });
  ...
  describe('currying', function() {
    it('should do nothing to lambdas with zero parameters', function() {
      expect(unannotate(t.curry(t.parse('(\\ () (a b 3))')))).toEqual(["\\", [], ["a", "b", "3"]]);
    });

    it('should do nothing to lambdas with one parameter', function() {
      expect(unannotate(t.curry(t.parse('(\\ (a) (a b 3))')))).toEqual(["\\", ["a"], ["a", "b", "3"]]);
    });

    it('should curry lambdas with two parameters', function() {
      expect(unannotate(t.curry(t.parse('(\\ (a b) (a b 3))')))).toEqual(["\\", ["a"], ["\\", ["b"], ["a", "b", "3"]]]);
    });

    it('should curry lambdas with multiple parameters', function() {
      expect(unannotate(t.curry(t.parse('(\\ (a b c) (a b 3))')))).toEqual(["\\", ["a"], ["\\", ["b"], ["\\", ["c"], ["a", "b", "3"]]]]);
    });
  });

  describe('de-let', function() {
    it('should rename let bound identifiers', function() {
      expect(unannotate(t.delet(t.alphaConvert(t.parse('(let ((a 1) (b 2)) (\\ () (a b)))'))))).toEqual(["\\", [], ["1", "2"]]);
    });

    it('should rename let and lambda bound identifiers', function() {
      expect(unannotate(t.delet(t.alphaConvert(t.parse('(let ((a 1) (b 2)) (\\ (b) (a b)))'))))).toEqual(["\\", ["b"], ["1", "b"]]);
    });
  });

  describe('beta reduction', function() {
    it('should apply all possible arguments', function() {
      expect(unannotate(t.betaReduce(t.alphaConvert(t.parse('((\\ (a) (\\ (b) (a b))) (c d))'))))).toEqual(["\\", ["b"], [["c", "d"], "b"]]);
    });

    it('should apply all possible arguments', function() {
      expect(unannotate(t.betaReduce(t.alphaConvert(t.parse('((\\ (a) (\\ (b) (a b))) (c d) (q x))'))))).toEqual([["c", "d"], ["q", "x"]]);
    });

    it('should apply all possible arguments', function() {
      expect(unannotate(t.betaReduce(t.alphaConvert(t.parse('((\\ (a) (\\ (b) (a b))) (c d) (q x) (p o))'))))).toEqual([[["c", "d"], ["q", "x"]], ["p", "o"]]);
    });
  });
  ...
  describe('interpret', function() {
    describe('lists', function() {
      ...
      it('should return false', function() {
        expect(unannotate(t.interpret(t.parse(`
        (let ((TRUE (\\ (x y) x))
              (FALSE (\\ (x y) y))
              (AND (\\ (x y) (x y x))))
             (AND TRUE FALSE))`
